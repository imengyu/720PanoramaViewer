...
{
	if (!loadingCanceled && pendingTexture == nullptr && queue.size() > 0) {
		...
		if (result) {
			if (result->success && !loadingCanceled) {
...
{
	if (!loadingCanceled && pendingLoadDataTexture != nullptr) {
...
void CCTextureLoadQueue::ForceCancelLoading()
{
